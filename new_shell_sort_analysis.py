...
def shellsort(a, seq, without_k1=False):
    """
    :param seq: 1 - shell
                2 - pratt1
                3 - pratt2
    ...
    for gap in choose_seq(seq, a):
        ...
        plot_hist(fig, seq, gap, all_values, all_vect_values)


def plot_hist(fig, seq, gap, all_values, all_vect_values):
    ax1 = fig.add_subplot(121)
    ax2 = fig.add_subplot(122)
    sns.distplot(all_vect_values, kde=False, label='Vectorized', ax=ax2, color='#ff8080')
    pal2 = sns.set_palette('PuBuGn_r')
    sns.distplot(all_values, kde=False, label='Original', ax=ax1, color=pal2)
    ax1.set_title('Исходный', size=15)
    ax2.set_title('Векторизованный', size=15)
    plt.subplots_adjust(wspace=0.3)
    plt.savefig('figures/' + get_name(seq) + ' k_' + str(gap)+'.pdf',format='pdf')
    plt.clf()
